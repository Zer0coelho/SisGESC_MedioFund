...
SELECT
    ...
    MAX(n.status) AS status_bimestre-- MAX ordena por valor alfabético: 'Reprovado' > 'Recuperacao' > 'Aprovado'
                                    -- A precedência correta é garantida por essa ordem lexicográfica:
                                    -- R-e-p = Reprovado (pior) › R-e-c = Recuperacao › A = Aprovado (melhor)
                                    -- Logo MAX retorna sempre o status mais grave do bimestre, que é o comportamento desejado.
FROM tb_notas n
...
CREATE TABLE tb_receitas (
    ...
    CONSTRAINT fk_rec_func FOREIGN KEY (fk_cpf_funcionario)
        REFERENCES tb_funcionarios (pk_cpf)
);-- colcar um if para um dos dois add +1
...
-- garante que só o primeiro vínculo (salario maior) registrado é usado, evitando duplicação de linhas
-- caso o funcionário possua mais de um cargo cadastrado.
WHERE v.cargo = (
...
Ver tabela e View e Trigger: ferias 
...
-- EX: mesmo dia + número de aula + ano letivo ao mesmo tempo.
-- apenas registros ativos (data_fim IS NULL).
...
BEGIN
  IF EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Erro: Professor ja esta alocado neste dia e horario!';
  END IF;
END$$
