...
-- no VIEW vw_contrato_escolar (definida no final deste script).
...
-- vw_ferias_detalhada: A "inteligência" que substitui o trigger
-- Calcula tudo em tempo real sem salvar lixo no banco
CREATE VIEW vw_ferias_detalhada AS
SELECT 
    f.fk_cpf_funcionario,
    f.ano,
    f.data_inicio,
    DATE_ADD(f.data_inicio, INTERVAL 29 DAY) AS data_fim, -- Calcula o fim das férias (29 dias após o início)
    DATE_ADD(f.data_inicio, INTERVAL 30 DAY) AS data_retorno, -- Calcula o retorno (30 dias após o início)
    v.salario_base / 3 AS valor_abono_ferias, -- Busca o salário no vínculo e calcula 1/3
    f.status,
    f.aprovado_por
FROM tb_ferias f
...
-- AAVISO: add mais view se for necessaria alterar outras tabelas se for necessario
/* ADD ESSE TRIGGER QUE É RELACIONADO COM A TB_FOLHA_PAGAMENTO

-- ============================================================
-- TRIGGER calcular_folha  (dentro do bloco DELIMITER $$)
-- ============================================================
-- Responsabilidade única: gravar o registro histórico da folha.
-- 1. Busca o salário vigente do funcionário em tb_vinculos e
--    salva em salario_base (valor do salário na competência).
-- 2. Verifica se há férias registradas para o funcionário
--    naquele mês/ano via vw_ferias_detalhada e salva o abono.
--    Se não houver férias no mês, abono_ferias fica como zero.
--
-- Nenhum cálculo de INSS, IRRF ou totais é feito aqui —
-- isso é responsabilidade exclusiva de vw_folha_pagamento.
-- ============================================================
...
-- ------------------------------------------------------------------------------------ --
-- VIEWS --
-- [3FN] Campos calculados expostos via VIEW em vez de armazenados fisicamente.
...
SELECT
  ...
  CASE
    WHEN m.status = 'Atrasado'
    THEN m.valor_mensalidade * (0.00033 * DATEDIFF(CURDATE(), m.data_vencimento))
    ELSE 0
  END AS juros,
  -- Valor total devido
  CASE
...
DELIMITER $$ -- O separador de comandos vai mudar temporariamente de ; para $$
...
-- [3FN] Agora calcula valor_final localmente (sem depender de campo armazenado no contrato).
-- Janeiro inclui rematrícula. Vencimento = dia 06 do mês seguinte ao mês de referência.
...
-- [3FN] Agora compara com valor_mensalidade (base) pois valor_total foi removido da tabela física.
--       Valores de multa e juros são calculados pela vw_mensalidades para referência,
--       mas a quitação é validada pelo sistema de pagamento com o valor correto.
...
-- Snapshot = Salário base no momento da emissão da folha yoru
-- calcular_folha grava o snapshot do salário atual e o abono de férias (se houver).
-- Todos os cálculos derivados (INSS, IRRF, descontos, líquido) ficam na vw_folha_pagamento.
CREATE TRIGGER calcular_folha
BEFORE INSERT ON tb_folha_pagamento
FOR EACH ROW
BEGIN
  DECLARE salario      DECIMAL(10,2);
  DECLARE abono        DECIMAL(10,2) DEFAULT 0;
  -- Snapshot do salário vigente no momento do INSERT
  SELECT salario_base INTO salario
  FROM tb_vinculos
  WHERE fk_cpf_funcionario = NEW.fk_cpf_funcionario
  LIMIT 1;
  SET NEW.salario_base = salario;
  -- Abono de férias do mês, calculado a partir de vw_ferias_detalhada
  SELECT COALESCE(valor_abono_ferias, 0) INTO abono
  FROM vw_ferias_detalhada
  WHERE fk_cpf_funcionario = NEW.fk_cpf_funcionario
    AND ano                = NEW.ano_referencia
    AND MONTH(data_inicio) = NEW.mes_referencia
  LIMIT 1;
  SET NEW.abono_ferias = abono;
END$$
